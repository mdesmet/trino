        ...
        implements PlanOptimizer
{
            ...
            extends SimplePlanRewriter<Set<Symbol>>
    {
        ...
        public PlanNode visitMergeProcessor(MergeProcessorNode node, RewriteContext<Set<Symbol>> context)
        {
            ...
            expectedInputsBuilder.add(node.getRowIdSymbol(), node.getMergeRowSymbol());
            expectedInputsBuilder.addAll(node.getRedistributionColumnSymbols());
            expectedInputsBuilder.addAll(node.getDataColumnSymbols());
            PlanNode source = context.rewrite(node.getSource(), expectedInputsBuilder.build());
            return new MergeProcessorNode(node.getId(), source, node.getTarget(), node.getRowIdSymbol(), node.getMergeRowSymbol(), node.getDataColumnSymbols(), node.getRedistributionColumnSymbols(), node.getOutputSymbols());
        }
